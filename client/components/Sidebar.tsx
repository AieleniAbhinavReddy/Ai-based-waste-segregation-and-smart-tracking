...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  // Use authenticated user data, demo data, or show loading state
  const demoProfile = isDemoMode && demoUser
    ? {
        full_name: demoUser.name || "Demo User",
        level: demoUser.role === "ADMIN" ? "Admin" : demoUser.role === "SUPERVISOR" ? "Supervisor" : "Eco Warrior",
        points: demoUser.role === "ADMIN" ? 9999 : demoUser.role === "SUPERVISOR" ? 5000 : 250,
        waste_classified: 47,
      }
    : null;
  const userData = isDemoMode ? demoProfile : (profile || null);
  const isLoading = isDemoMode ? false : (profileLoading || !sbUser);
