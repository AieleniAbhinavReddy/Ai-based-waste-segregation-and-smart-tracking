...
export default function Dashboard() {
  ...
  // Helper: safely extract a plain name string
  const getDisplayName = (raw: any): string => {
    if (!raw) return "User";
    if (typeof raw === "string") {
      // Handle JSON-encoded object like '{"full_name": "Abhinav Reddy"}'
      if (raw.trim().startsWith("{")) {
        try {
          const parsed = JSON.parse(raw);
          return parsed.full_name || parsed["full name"] || parsed.name || raw;
        } catch {
          return raw;
        }
      }
    ...
    if (typeof raw === "object") {
      return raw.full_name || raw["full name"] || raw.name || "User";
    }
    return String(raw);
  };
  ...
  const demoProfile = isDemoMode
    ? {
        full_name: demoUser?.name || "Abhinav Reddy",
        level: "Eco Warrior",
        eco_score: 85,
        points: 250,
        waste_classified: 47,
  ...
  // Show loading state while fetching data (not in demo mode)
  ...
  // Ensure we have valid user data (skip check in demo mode)
  if (!isDemoMode && !user) return null;
  if (!isDemoMode && !profile) return null;

  // Use authenticated user data, or demo mock data
  const userData = isDemoMode ? demoProfile : profile;
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">
              Welcome back, {getDisplayName(userData.full_name)}! 👋
            </h1>
